...
var $widths;
	...
	function CheckPageBreak($h, $setX) {
		...
		if ($this->GetY() + $h > $this->PageBreakTrigger) {
			...
			$this->Cell(44, 8, 'Contraseña', 1, 0, 'C', 0);
			$this->Cell(40, 8, 'Correo', 1, 1, 'C', 0);
...
$pdf->Cell(44,8,utf8_decode('contraseña'),'B',0,'C',0);
$pdf->Cell(40,8,'Correo','B',1,'C',0);

$pdf->SetFillColor(233, 229, 235);//color de fondo rgb
$pdf->SetDrawColor(61, 61, 61);//color de linea  rgb
$pdf->SetWidths(array(6, 60, 40, 44, 40)); //???
// esto no lo mencione en el video pero también pueden poner la alineación de cada COLUMNA!!!
$pdf->SetAligns(array('C','C','C','C'));
$pdf->SetFont('Arial','',12);
$save=conectar();
$resultado=$save->prepare("SELECT * FROM administrador");
$resultado->execute();
$results=$resultado->fetchAll(PDO::FETCH_ASSOC);
for ($i = 0; $i < count($results); $i++) {

	$pdf->Row(array(ucwords(strtolower(utf8_decode($results[$i]['id']))),ucwords(strtolower(utf8_decode($results[$i]['nombre']))),$results[$i]['apellido'],ucwords(strtolower(utf8_decode("*********"))), $results[$i]['correo']), 10);
}
$resultado=null;
$save=null;
